...
public partial class SettingsPage : ContentPage
{
    ...
    {
        // Unlocked edition: no account, no billing, no plan limits.
        var current = Subscriptions.Current;
        SubscriptionLabel.Text = $"Edition: {current.Name} - every feature included";
    ...
    // The unlocked edition has no account to sign out of, so there is no
    // logout action: the app never asks for credentials in the first place.
}